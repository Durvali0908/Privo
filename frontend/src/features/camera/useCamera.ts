...
export type CameraScreenState =
    ...
    | "result"         // analysis complete — show findings
    | "protecting"    // user reviewing risks before save
...
export interface UseCameraReturn {
    ...
    /** Move from result → protecting state. */
    handleProceedToProtect: () => void;

    /** Skip protection and go straight to save. */
    handleSkipProtection: () => void;

    /** Save captured image to Privo Gallery (Option A: original image). */
...
export function useCamera(): UseCameraReturn {
    ...
    const handleProceedToProtect = useCallback(() => {
        setScreenState("protecting");
    }, []);

    const handleSkipProtection = useCallback(() => {
        setScreenState("protecting");
    }, []);

    const handleSave = useCallback(async () => {
        // Actual file copy + SQLite write handled by useCameraSession.
        // useCamera signals intent only — useCameraSession owns persistence.
